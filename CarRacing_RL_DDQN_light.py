...
class Brain:
    ...
    def _createModel(self):
        ...
        x = Dense(self.actionCnt, activation="softmax")(x)
...
UPDATE_TARGET_FREQUENCY = int(2e2)  #of of learning agent's steps
...
class Agent:
    ...
    def _getTargets(self, batch):
        ...
        for i in range(len(batch)):
            ...
            self.x[i] = s
            #self.y[i] = f_softmax(t)
            #self.y[i] = np.zeros_like(t)
            #self.y[i][t.argmax()] = 1 
            self.y[i] = self._to_onehot(t.argmax())
            
            #sec_bets = t[np.argsort(t)[::-1][1]]
            if self.steps % 20 == 0 and i == len(batch)//2:
                print('a',a, 't',t[a], 'r: %.4f' % r,'mean t',np.mean(t))
                
            #Backprogate dicrtly on proba differences
            #If nok, check on hot vect: y = 0; y[argmax(t)] = 1 and loss categorical_crossentropy 
